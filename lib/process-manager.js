...
class ProcessManager {
  ...
  runInteractiveClaude(projectId, projectPath) {
    this.stopClaude(projectId);
    let child;
    try {
      ...
      this.emit({ type: 'error', source: 'terminal', projectId, text: error.message, error });
      return null;
    }
    this.processes.set(`terminal:${projectId}`, child);
    this.emit({ type: 'started', source: 'terminal', projectId });
    this.attach(child, 'terminal', projectId);
    return child;
  }

  sendInteractiveInput(projectId, input) {
    const child = this.processes.get(`terminal:${projectId}`);
    if (!child || !child.stdin || !child.stdin.write) return false;
  ...
  handleOutput(source, projectId, text) {
    if (source === 'terminal') {
      this.emit({ type: 'output', source, projectId, text });
      return;
    }
    if (source === 'dev') {
  ...
  stopClaude(projectId) {
    ...
    return this.stopInteractiveClaude(projectId) || stopped;
  }
  stopInteractiveClaude(projectId) { return this.stopProcess('terminal', projectId); }
