...
class ProcessManager {
  ...
  runInteractiveClaude(projectId, projectPath, terminalId, sessionId) {
    ...
    if (nodePty) return this.runNodePty(projectId, projectPath, id, this.claudeCommand, args);
    return this.runFallbackTerminal(projectId, projectPath, id, this.claudeCommand, args);
  }

  runInteractiveShell(projectId, projectPath, terminalId) {
    const id = terminalId || `terminal-${Date.now()}-${++this.terminalCounter}`;
    this.stopInteractiveClaude(projectId, id);
    const command = process.platform === 'win32' ? (process.env.ComSpec || 'cmd.exe') : (process.env.SHELL || '/bin/bash');
    const args = process.platform === 'win32' ? [] : ['-i'];
    if (nodePty) return this.runNodePty(projectId, projectPath, id, command, args);
    return this.runFallbackTerminal(projectId, projectPath, id, command, args);
  }

  profile(projectPath) { return detectProjectProfile(projectPath, this.fs); }

  runNodePty(projectId, projectPath, terminalId, command, args) {
    let terminal;
    try {
  ...
  runFallbackTerminal(projectId, projectPath, terminalId, command, args) {
    let child;
    try {
      if (process.platform === 'linux') {
        const commandLine = [command].concat(args).map(shellQuote).join(' ');
        child = this.spawn('script', ['-qefc', commandLine, '/dev/null'], {
          cwd: projectPath,
          shell: false,
          detached: true
        });
      } else {
        child = this.spawn(command, args, { cwd: projectPath, shell: false });
      }
    } catch (error) {
      this.emit({ type: 'error', source: 'terminal', projectId, terminalId, text: error.message, error });
      return null;
