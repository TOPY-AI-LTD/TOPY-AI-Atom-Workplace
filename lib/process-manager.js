...
let nodePty = null;
try { nodePty = require('node-pty'); } catch (error) { nodePty = null; }
...
function detectProjectProfile(projectPath, fileSystem) {
  try {
    ...
    return { framework, packageManager: detectPackageManager(projectPath, fileSystem).name, devPort: portMatch ? Number(portMatch[1]) : null };
  } catch (error) {
    return { framework: 'unknown', packageManager: detectPackageManager(projectPath, fileSystem).name, devPort: null };
  }
}
...
class ProcessManager {
  ...
  runInteractiveClaude(projectId, projectPath, terminalId) {
    const id = terminalId || `terminal-${Date.now()}-${++this.terminalCounter}`;
    this.stopInteractiveClaude(projectId, id);
    if (nodePty) return this.runNodePty(projectId, projectPath, id);
    return this.runFallbackTerminal(projectId, projectPath, id);
  }

  profile(projectPath) { return detectProjectProfile(projectPath, this.fs); }

  runNodePty(projectId, projectPath, terminalId) {
    let terminal;
    try {
      terminal = nodePty.spawn(this.claudeCommand, [], {
        cwd: projectPath,
        name: 'xterm-color',
        cols: 120,
        rows: 32,
  ...
  runFallbackTerminal(projectId, projectPath, terminalId) {
    let child;
    try {
      if (process.platform === 'linux') {
        child = this.spawn('script', ['-qefc', shellQuote(this.claudeCommand), '/dev/null'], {
          cwd: projectPath,
          shell: false,
          detached: true
        });
      } else {
        child = this.spawn(this.claudeCommand, [], { cwd: projectPath, shell: false });
      }
    } catch (error) {
      this.emit({ type: 'error', source: 'terminal', projectId, terminalId, text: error.message, error });
      return null;
  ...
  sendInteractiveInput(projectId, input, terminalId) {
