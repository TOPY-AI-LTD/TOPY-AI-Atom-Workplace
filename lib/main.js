...
function openPreview() {
  const project = requireProject();
  if (!project) return;
  const url = processes.url(project.id) || `http://localhost:${project.devPort || 3000}`;
  showPanel();
...
function openExternalPreview() {
  const project = requireProject();
  if (!project) return;
  const url = (view && view.previewUrl()) || processes.url(project.id) || `http://localhost:${project.devPort || 3000}`;
