...
function startInteractiveSession() {
  const project = requireProject();
  if (!project) return;
  activeTerminalId = processes.runInteractiveClaude(project.id, project.path);
  showPanel();
}
...
function sendTerminalInput() {
  ...
  if (!processes.sendInteractiveInput(project.id, view.promptText(), view.activeTerminalId() || activeTerminalId)) {
