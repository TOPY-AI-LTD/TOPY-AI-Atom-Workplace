...
function activate() {
  ...
  processes = new ProcessManager(handleProcessEvent, {
    claudeCommand: configuredClaudeCommand()
  });
  ...
  subscriptions = atom.commands.add('atom-workspace', {
    ...
    'claude-workspace:start-shell': startShell,
...
function configuredClaudeCommand() {
  const configured = atom.config.get('claude-workspace.claudeCommand') || 'claude';
  if (configured !== 'claude') return configured;
  const localCommand = path.join(process.env.HOME || '', '.local', 'bin', 'claude');
  return fs.existsSync(localCommand) ? localCommand : configured;
}
...
function createView() {
  view = new WorkspaceView({
    ...
    onAction: (action, ...args) => {
      ...
      if (!handler) return;
      try { return handler(...args); } catch (error) { notifyError(`Claude Workspace action failed: ${action}`, error.message); }
    }
  });
  return view;
}
...
function startInteractiveSession() {
  ...
  if (!activeTerminalId) notifyError('Claude terminal did not start', 'Check that the claude command is installed and available in Atom PATH.');
  showPanel();
}
...
function startShell() {
  const project = requireProject();
  if (!project) return;
  activeTerminalId = processes.runInteractiveShell(project.id, project.path);
  if (!activeTerminalId) notifyError('Shell terminal did not start', 'Check that /bin/bash or your configured shell is available.');
  showPanel();
}
