class WorkspaceView {
  constructor({ onProjectChange, onAction, onPromptChange, onTerminalInput, maxOutputChars, sidebar }) {
    this.onAction = onAction || (() => {});
    this.onPromptChange = onPromptChange || (() => {});
    this.onTerminalInput = onTerminalInput || (() => {});
    this.sidebar = Boolean(sidebar);
    ...
    const fullTemplate = `
      ...
      <section class="claude-workspace__sessionbar">
        ...
        <button class="btn" data-action="rename-session">Rename</button>
        <button class="btn" data-action="send-terminal-input">Send input</button>
        <button class="btn" data-action="interrupt-terminal">Ctrl-C</button>
        ...
        <button class="btn" data-action="clear-sessions">Clear history</button>
      ...
      <div class="claude-workspace__body">
        ...
        <div class="claude-workspace__preview-wrap">
          <div class="claude-workspace__previewbar">
            ...
            <span class="claude-workspace__preview-url"></span>
          ...
          <webview class="claude-workspace__webview" title="Local development preview" allowpopups hidden></webview>
          <div class="claude-workspace__preview-empty">Start a dev server to preview your app.</div>
        </div>
      </div>`;
    this.element.innerHTML = this.sidebar ? `
      <section class="claude-workspace__sidebar-controls">
        <select id="claude-workspace-projects" class="claude-workspace__projects" title="Project"></select>
        <div class="claude-workspace__session-controls">
          <select id="claude-workspace-sessions" class="claude-workspace__sessions" title="Session"><option value="">New session</option></select>
          <button class="btn" data-action="start-shell">Shell</button>
          <button class="btn btn-primary" data-action="start-terminal">Start / Resume</button>
        </div>
      </section>
      <div class="claude-workspace__sidebar-terminal">
        <div class="claude-workspace__terminalbar" hidden>
          <label for="claude-workspace-terminals">Terminal</label>
          <select id="claude-workspace-terminals" class="claude-workspace__terminals"></select>
          <button class="btn btn-sm" data-action="interrupt-terminal">Ctrl-C</button>
          <button class="btn btn-sm" data-action="stop-terminal">Stop</button>
        </div>
        <div class="claude-workspace__terminal-placeholder">Start Claude to begin a conversation.</div>
        <div class="claude-workspace__terminal"></div>
        <textarea class="claude-workspace__terminal-input" rows="2" placeholder="Type a message and press Enter..."></textarea>
      </div>
      <pre class="claude-workspace__output" hidden></pre>
      <select class="claude-workspace__log-filter" hidden><option value="all">All output</option></select>
      <div class="claude-workspace__status" data-status="ready" hidden><span class="claude-workspace__status-label">Ready</span></div>
      <iframe class="claude-workspace__preview" hidden></iframe>
      <webview class="claude-workspace__webview" hidden></webview>
      <div class="claude-workspace__preview-wrap" hidden><span class="claude-workspace__preview-url"></span><div class="claude-workspace__preview-empty"></div></div>
    ` : fullTemplate;
    ...
    this.terminalInput = this.element.querySelector('.claude-workspace__terminal-input');
    this.terminalPlaceholder = this.element.querySelector('.claude-workspace__terminal-placeholder');
    ...
    this.webview = this.element.querySelector('.claude-workspace__webview');
    this.previewUrlLabel = this.element.querySelector('.claude-workspace__preview-url');
    ...
    if (this.preview) this.preview.addEventListener('load', () => this.onAction('preview-loaded'));
    if (this.preview) this.preview.addEventListener('error', () => this.onAction('preview-error'));
    if (this.webview) {
      this.webview.addEventListener('did-finish-load', () => this.onAction('preview-loaded'));
      this.webview.addEventListener('console-message', event => this.onAction('preview-console', event.message));
      this.webview.addEventListener('did-fail-load', event => this.onAction('preview-error', event.errorDescription));
    }
    ...
    if (this.terminalInput) {
      this.terminalInput.addEventListener('keydown', event => {
        if (event.key === 'Enter' && !event.shiftKey) {
          ...
          this.onTerminalInput(this.terminalInput.value);
          this.terminalInput.value = '';
        }
      });
    }
    if (this.logFilter) this.logFilter.addEventListener('change', event => this.renderOutput(event.target.value));
    if (this.terminals) this.terminals.addEventListener('change', event => this.showTerminal(event.target.value));
  ...
  setProjects(projects, activeId) {
    this.projects.innerHTML = projects.length
      ? projects.map(project => {
        const state = project.status ? ` · ${project.status}` : '';
        const sessions = Number(project.sessionCount) ? ` · ${project.sessionCount} sessions` : '';
        const git = project.git ? ` · ${project.git.branch}${project.git.dirty ? '*' : ''}` : '';
        return `<option value="${escapeHtml(project.id)}" title="${escapeHtml(project.path)}" ${project.id === activeId ? 'selected' : ''}>${escapeHtml(project.name)}${escapeHtml(state)}${escapeHtml(sessions)}${escapeHtml(git)}</option>`;
      }).join('')
  ...
  setSessions(sessions, selectedId) {
    this.sessions.innerHTML = '<option value="">New session</option>' + (sessions || []).map(session =>
      `<option value="${escapeHtml(session.id)}" ${session.id === selectedId ? 'selected' : ''}>${escapeHtml(session.title || session.id)}${session.lastUsedAt ? ` · ${escapeHtml(new Date(session.lastUsedAt).toLocaleString())}` : ''}</option>`
    ).join('');
  }

  selectedSessionId() { return this.sessions.value || null; }
  promptText() { return this.prompt ? this.prompt.value.trim() : ''; }
  terminalInputText() { return this.terminalInput ? this.terminalInput.value.trim() : ''; }
  ...
  showTerminal(id) {
    ...
    if (this.terminalPlaceholder) this.terminalPlaceholder.hidden = true;
    ...
    if (!terminalView) {
      ...
      try {
        ...
        instance.open(host);
        instance.onData(data => this.onTerminalInput(data));
        if (this.terminalInput) this.terminalInput.hidden = true;
        terminalView = { instance, host };
      } catch (error) {
        const pre = document.createElement('pre');
        pre.className = 'claude-workspace__terminal-fallback';
        host.appendChild(pre);
        if (this.terminalInput) this.terminalInput.hidden = false;
  ...
  showPreview(url) {
    ...
    this.previewUrlLabel.textContent = url;
    this.setStatus('Preview open', 'running');
  }

  refreshPreview() {
    if (this.preview.dataset.url) this.preview.src = this.preview.dataset.url;
    if (this.webview && !this.webview.hidden && this.webview.reload) this.webview.reload();
  }
