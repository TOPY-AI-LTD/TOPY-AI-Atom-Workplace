  ...
  setProjects(projects, activeId) {
    this.projects.innerHTML = projects.length
      ? projects.map(project => {
        ...
        return `<option value="${escapeHtml(project.id)}" title="${escapeHtml(project.path)}" ${project.id === activeId ? 'selected' : ''}>${escapeHtml(project.name)}${escapeHtml(state)}${escapeHtml(sessions)}</option>`;
      }).join('')
