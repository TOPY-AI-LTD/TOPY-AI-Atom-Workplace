...
- 在支持 Electron `webview` 的 Atom 环境捕获预览 console 和加载失败事件，旧环境回退到 iframe；
...
- 停止时只终止插件自己创建的子进程；
- 插件退出时清理子进程；
- Linux 当前通过进程组处理子进程树；Windows/macOS 仍需专门的跨平台进程树适配。
...
- [x] 自动健康检查；
- [x] 刷新和重新连接；
- [x] Electron webview console 输出和加载失败诊断（不支持时 iframe 回退）；
- [x] HTTP 健康检查和网络错误提示；
